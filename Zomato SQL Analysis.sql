...
select a.userid, SUM(b.price) as "Total Amount Spent" from sales a
...
select * from 
(select *, RANK() over(partition by userid order by created_date) as "Rank" from sales) a where Rank = 1
